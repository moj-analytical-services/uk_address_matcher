...
def create_test_bundle(root: Path) -> Path:
    ...
    try:
        connection.execute(
            ...
            'E1 1AA' messy_postcode, NULL::VARCHAR ukam_label, FALSE has_existing_label,
            ...
            [{'rank': 1::BIGINT, 'label_id': 'label-1'::VARCHAR},
             {'rank': 2::BIGINT, 'label_id': 'label-2'::VARCHAR}] top_candidates
            UNION ALL
            ...
            [{'rank': 1::BIGINT, 'label_id': 'label-3'::VARCHAR}] top_candidates
            ) TO ? (FORMAT PARQUET)""",
            [str(data_file)],
        )
    finally:
